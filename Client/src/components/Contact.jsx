...
const Contact = () => {
    ...
    return (
        <div className="bg-base-200 flex flex-col items-center py-4" id="contact">
            <div className="max-w-screen-xl mx-auto px-4 py-2">
                {/* Composant d'alerte */}
                {alert.show && (
                    <div
                        role="alert"
                        className={`alert fixed top-4 right-4 max-sm:left-4 max-sm:right-4 z-50`}
                    >
                ...
                <fieldset className="fieldset bg-base-300 border-base-300 rounded-box w-full mx-auto p-8">
                    ...
                    <form className="contact_form" onSubmit={sendEmail} noValidate>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <input type="text" name="user_org" className={`input w-full border-primary ${errors.user_org ? 'input-error' : ''}`} placeholder="Organisation" />

                            <input type="email" name="user_email" className={`input w-full border-primary ${errors.user_email ? 'input-error' : ''}`} placeholder="Email" />

                            <textarea name="message" className={`textarea textarea-xl w-full border-primary py-0 ${errors.message ? 'textarea-error' : ''}`} placeholder="Message" minLength="10" />                    </div>
